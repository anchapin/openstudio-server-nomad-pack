...
- Fixed `wait_for_core_services` in `fresh-redeploy-openstack.sh` falsely reporting core services not ready when the latest Nomad deployment record is `"failed"` even though all task groups are running. On a fresh redeploy where the `web` job experiences transient failed allocations (prestart `wait-for-deps` timeout or health-check deadline) before eventually stabilising, Nomad retains the `"failed"` deployment status. The readiness check now falls back to checking the job summary: if all task groups have `Running >= 1` and `Queued == 0` the service is counted as ready regardless of the stale deployment record. (#457)
- Fixed repeated `web` allocation failures on fresh cold-start deploys caused by the 5-minute `web_update_healthy_deadline` being too short for MongoDB init + Rails startup on a freshly wiped CSI volume. Added explicit `web_update_healthy_deadline = "20m"` and `web_update_progress_deadline = "30m"` to `examples/advanced/openstack-production.hcl`. (#457)
- Fixed `nfs-preflight` prestart task always failing and blocking web startup. The task uses `driver = "raw_exec"` (runs on the host) but was checking `nfs_volume_mount_path` (`/mnt/openstudio`), which is the container-internal mount path and does not exist on the host filesystem. Additionally, `mountpoint -q` was used to verify the path, but the Nomad host volume path (`/nfs/openstudio/batch/openstudio`) is a subdirectory of the NFS mount — not the NFS mountpoint itself — so `mountpoint -q` always returns false even on a healthy NFS. Fixed by switching to `$NOMAD_HOST_DIR_nfs_shared` (the actual host-side path Nomad injects for all tasks in a group declaring the `nfs-shared` host volume) and replacing `mountpoint -q` with a plain `[ -d ] && [ -w ]` existence+writability check.
