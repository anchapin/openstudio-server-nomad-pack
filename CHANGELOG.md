...
- Consolidate tightly coupled services into unified Nomad jobs: merged `worker` task group into `web.nomad.tpl` under `<job_name>-web` and `stall-watchdog` task group into `queue-sweeper.nomad.tpl` under `<job_name>-queue-sweeper` while preserving all configuration options, service discovery stanzas, and template compatibility (#405).
