...
| `worker_autoscaling_max_scale_delta` | `number` | `200` | Maximum number of worker allocations to add or remove in a single autoscaler evaluation cycle. Prevents a mass scale-up event from flooding the cluster with thousands of new workers simultaneously when the Consul agent is unable to handle the sudden burst of template-engine requests (Consul 429 thundering-herd). Recommended: 200 for large fleets (5,000+ workers). Set to 0 to disable the delta limit (Nomad autoscaler default behavior). Only applies when worker_autoscaling_enabled = true. |
